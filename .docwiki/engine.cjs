...
function slugify(input) {
  return input.normalize("NFD").replace(new RegExp("\\p{Diacritic}", "gu"), "").toLowerCase().trim().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
}

// src/manifest.ts
var manifestDocNodeSchema = external_exports.object({
  type: external_exports.literal("doc"),
  id: external_exports.string().min(1)
});
var manifestNodeSchema = external_exports.lazy(
  () => external_exports.union([
    manifestDocNodeSchema,
    external_exports.object({
      type: external_exports.literal("category"),
      label: external_exports.string().min(1),
      items: external_exports.array(manifestNodeSchema).min(1)
    })
  ])
);
var manifestSchema = external_exports.object({
  version: external_exports.number().int().positive(),
  tree: external_exports.array(manifestNodeSchema)
});
function validateManifest(raw2) {
  return manifestSchema.parse(raw2);
}
function flattenDocIds(manifest) {
  const ids = [];
  function walk(nodes) {
...
function buildIdToUrlMap(manifest) {
  const map2 = /* @__PURE__ */ new Map();
  function walk(nodes, prefix) {
    for (const node2 of nodes) {
      if (node2.type === "doc") {
        map2.set(node2.id, `${prefix}/${node2.id}`);
      } else {
        walk(node2.items, `${prefix}/${slugify(node2.label)}`);
      }
    }
  }
  walk(manifest.tree, "");
...
function findDuplicateIds(manifest) {
  const seen = /* @__PURE__ */ new Set();
  const duplicates = /* @__PURE__ */ new Set();
  for (const id of flattenDocIds(manifest)) {
...
var deserializer = ($, _) => {
  ...
  const unpair = (index2) => {
    ...
    return as(guard(type2, value), index2);
  };
  return unpair;
};
...
var frontmatterSchema = external_exports.object({
  id: external_exports.string().min(1, "frontmatter.id \xE9 obrigat\xF3rio"),
  title: external_exports.string().min(1, "frontmatter.title \xE9 obrigat\xF3rio"),
  description: external_exports.string().optional(),
  sidebar_label: external_exports.string().optional(),
...
function validateFrontmatter(raw2, filePath) {
  const result = frontmatterSchema.safeParse(raw2);
  if (!result.success) {
    const details = result.error.issues.map((issue) => `${issue.path.join(".")}: ${issue.message}`).join("; ");
    throw new Error(`Frontmatter inv\xE1lido em ${filePath}: ${details}`);
  }
  return result.data;
}
...
// src/links.ts
var WIKILINK_RE = /\[\[([^\]|]+?)(?:\|([^\]]+?))?\]\]/g;
function remarkWikiLinks(idToUrl, onBrokenLink) {
  return function transformer(tree) {
    visit(tree, "text", (node2, index2, parent) => {
      if (index2 === void 0 || !parent) return;
      if (!node2.value.includes("[[")) return;
      const replacement = [];
      let lastIndex = 0;
      let match;
      WIKILINK_RE.lastIndex = 0;
      while ((match = WIKILINK_RE.exec(node2.value)) !== null) {
        const [full, id, label] = match;
        const trimmedId = id.trim();
        if (match.index > lastIndex) {
          replacement.push({
            type: "text",
            value: node2.value.slice(lastIndex, match.index)
          });
        }
        const url = idToUrl.get(trimmedId);
        if (url) {
          replacement.push({
            type: "link",
            url,
            children: [{ type: "text", value: (label ?? trimmedId).trim() }]
          });
        } else {
          onBrokenLink(trimmedId);
          replacement.push({
            type: "html",
            value: `<span class="broken-link" data-target="${trimmedId}">${(label ?? trimmedId).trim()}</span>`
          });
        }
        lastIndex = match.index + full.length;
      }
      if (lastIndex < node2.value.length) {
        replacement.push({
          type: "text",
          value: node2.value.slice(lastIndex)
        });
      }
...
function requireSnippet() {
  ...
  function makeSnippet(mark2, options) {
    ...
    for (let i = 1; i <= options.linesBefore; i++) {
      ...
      result = common2.repeat(" ", options.indent) + padStart((mark2.line - i + 1).toString(), lineNoLength) + " | " + line2.str + "\n" + result;
    }
    const line = getLine(mark2.buffer, lineStarts[foundLineNo], lineEnds[foundLineNo], mark2.position, maxLineLength);
    result += common2.repeat(" ", options.indent) + padStart((mark2.line + 1).toString(), lineNoLength) + " | " + line.str + "\n";
...
function requireSchema() {
  ...
  function Schema22(definition2) {
    return this.extend(definition2);
  }
  Schema22.prototype.extend = function extend2(definition2) {
    let implicit = [];
    let explicit = [];
    if (definition2 instanceof Type2) {
...
function requireLoader() {
  ...
  function rollbackAnchorTransaction(state) {
    ...
    for (let index2 = names.length - 1; index2 >= 0; index2 -= 1) {
      ...
      if (entry.existed) {
        state.anchorMap[names[index2]] = entry.value;
      } else {
        delete state.anchorMap[names[index2]];
      }
    }
  }
...
function requireDumper() {
  ...
  function compileStyleMap(schema2, map2) {
    ...
    for (let index2 = 0, length = keys2.length; index2 < length; index2 += 1) {
  ...
  function encodeHex(character) {
    ...
    return "\\" + handle2 + common2.repeat("0", length - string3.length) + string3;
  }
  ...
  function testImplicitResolving(state, str2) {
    for (let index2 = 0, length = state.implicitTypes.length; index2 < length; index2 += 1) {
      const type2 = state.implicitTypes[index2];
...
// src/splitFrontmatter.ts
var FRONTMATTER_RE = /^---\r?\n([\s\S]*?)\r?\n---\r?\n?/;
function splitFrontmatter(raw2) {
  const match = raw2.match(FRONTMATTER_RE);
  if (!match) {
    return { data: {}, content: raw2 };
  }
  const data = load(match[1]) ?? {};
  const content3 = raw2.slice(match[0].length);
  return { data, content: content3 };
}

// src/parseDoc.ts
async function parseDoc(rawFile, filePath, idToUrl) {
  const { data, content: content3 } = splitFrontmatter(rawFile);
  const frontmatter = validateFrontmatter(data, filePath);
  const brokenLinks = [];
  const file = await unified().use(remarkParse).use(remarkWikiLinks, idToUrl, (id) => brokenLinks.push(id)).use(remarkRehype, { allowDangerousHtml: true }).use(rehypeStringify, { allowDangerousHtml: true }).process(content3);
  return {
    frontmatter,
    html: String(file),
    brokenLinks,
...
async function build(options) {
  ...
  const manifestRaw = JSON.parse(
    await (0, import_promises.readFile)(
      import_node_path2.default.join(options.contentDir, "docwiki.manifest.json"),
      "utf-8"
    )
  );
  const manifest = validateManifest(manifestRaw);
  for (const dup of findDuplicateIds(manifest)) {
    issues.push({
      level: "error",
      message: `Id "${dup}" aparece mais de uma vez no manifest`,
      docId: dup
    });
  }
  const idToUrl = buildIdToUrlMap(manifest);
  const manifestIds = new Set(flattenDocIds(manifest));
  const markdownFiles = await findMarkdownFiles(options.contentDir);
  const parsedDocs = [];
  const seenIds = /* @__PURE__ */ new Set();
  for (const filePath of markdownFiles) {
    const raw2 = await (0, import_promises.readFile)(filePath, "utf-8");
    let parsed;
    try {
      parsed = await parseDoc(raw2, filePath, idToUrl);
    } catch (error) {
      issues.push({
        level: "error",
    ...
    if (seenIds.has(parsed.frontmatter.id)) {
      issues.push({
        level: "error",
        message: `Id "${parsed.frontmatter.id}" duplicado entre arquivos (${filePath})`,
        docId: parsed.frontmatter.id
      });
      continue;
    }
    seenIds.add(parsed.frontmatter.id);
    if (!manifestIds.has(parsed.frontmatter.id)) {
      issues.push({
        level: "warning",
        message: `Doc "${parsed.frontmatter.id}" (${filePath}) n\xE3o est\xE1 referenciado no manifest \u2014 ficar\xE1 \xF3rf\xE3o, sem entrada de navega\xE7\xE3o`,
        docId: parsed.frontmatter.id
      });
    }
    for (const brokenId of parsed.brokenLinks) {
      issues.push({
        level: "error",
        message: `Link quebrado em "${parsed.frontmatter.id}": [[${brokenId}]] n\xE3o existe`,
        docId: parsed.frontmatter.id
      });
    }
  ...
  for (const manifestId of manifestIds) {
    if (!seenIds.has(manifestId)) {
      issues.push({
        level: "error",
        message: `Manifest referencia id "${manifestId}", mas nenhum arquivo .md com esse id foi encontrado`,
        docId: manifestId
      });
    }
  }
  ...
  for (const doc of parsedDocs) {
    const url = idToUrl.get(doc.frontmatter.id);
    if (!url) continue;
    const outPath = import_node_path2.default.join(options.outDir, `${url}.html`);
    await (0, import_promises.mkdir)(import_node_path2.default.dirname(outPath), { recursive: true });
    await (0, import_promises.writeFile)(
      outPath,
      renderPage(doc.frontmatter.title, doc.html),
      "utf-8"
    );
    outputFiles.push(outPath);
  }
  return { outputFiles, issues };
}
...
// src/cli.ts
async function main() {
  const [, , contentDirArg, outDirArg] = process.argv;
  const contentDir = contentDirArg ?? ".";
  const outDir = outDirArg ?? "site";
  console.log(`[docwiki-engine] build: "${contentDir}" -> "${outDir}"`);
  const result = await build({ contentDir, outDir });
  const errors = result.issues.filter((i) => i.level === "error");
  const warnings = result.issues.filter((i) => i.level === "warning");
  for (const w of warnings) {
    console.warn(`[docwiki-engine] warning: ${w.message}`);
  }
  for (const e of errors) {
    console.error(`[docwiki-engine] error: ${e.message}`);
  }
  console.log(
    `[docwiki-engine] ${result.outputFiles.length} p\xE1gina(s) geradas, ${errors.length} erro(s), ${warnings.length} warning(s).`
  );
  if (errors.length > 0) {
    console.error(
      "[docwiki-engine] build falhou: corrija os erros acima antes de publicar."
    );
    process.exit(1);
  }
}
main().catch((error) => {
  console.error("[docwiki-engine] falha inesperada:", error);
  process.exit(1);
});
